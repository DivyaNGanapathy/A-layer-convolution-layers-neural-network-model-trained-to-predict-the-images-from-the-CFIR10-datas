...
Original file is located at
    https://colab.research.google.com/drive/1I1CE_u2r-vvc0nkd3x9PmM_OMuF6tC3n
"""
...
def AccuracyCheck(trainloader, testloader, net, criterion, optimizer, batch_size, classes):
    ...
    for epoch in range(50):  # Setting the epochs to 50 as suggested in the question
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            optimizer.zero_grad()
            outputs = net(inputs)
            LabelsModified = torch.FloatTensor(labels.shape[0], outputs.shape[1]).zero_()
#Labels are modified as for Mean Square Error function the input should not be a single value but one hot encoding vector
            LabelsModified = LabelsModified.to(device)
            for i in range(LabelsModified.shape[0]):
              for j in range(LabelsModified.shape[1]):
                if j==labels[i]:
                  LabelsModified[i][j] = 1    
            loss = criterion(outputs, LabelsModified)
#Back Propagation and optimizer adjustment(Learning)
            loss.backward()
            optimizer.step()
...
#Accuracy on TrainData Calculated and Printed
        ...
        accuracy_trainList.append(100 * correct / total)
...
#Accuracy on TestData Calculated and Printed
        ...
        accuracy_testList.append(100 * correct / total)
    ...
    return loss_lst,accuracy_trainList,accuracy_testList

def main():
			
#Mean Squared Error Loss function used instaed of Cross Entropy to calculate the loss function
        net = NetFCQ1Q4()
        ...
        draw('Loss',loss)
        draw('Train Acc.',Accuracy_trainset)
        draw('Test Acc',Accuracy_testset)
